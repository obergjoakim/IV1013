   ...
   */
    ...
    public void keyToDecodeMatrix(String keyfile, int blocksize, int radix) throws FileNotFoundException, Exception
    {
        try {
            ...
            DenseMatrix<Real> keyMatrixToInvert = DenseMatrix.valueOf(toMatrix_temp);

            int detOfKeyMatrix = keyMatrixToInvert.determinant().intValue();
          ...
          //  DenseMatrix<Real> inverse = keyMatrixToInvert.inverse();
            ...
            LargeInteger mulinv = LargeInteger.valueOf(multiplicative_invers);
            DenseMatrix<Real> adj = keyMatrixToInvert.adjoint();
           // DenseMatrix<Real> inverse = adj.times(LongInteger.valueOf(mulinv))
            ...
            for(int i=0;i<blocksize;i++)
            {
                ...
                {
                    LargeInteger mod = LargeInteger.valueOf(adj.get(i,j).longValue()).times(mulinv).mod(LargeInteger.valueOf(radix));
